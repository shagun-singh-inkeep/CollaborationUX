...
- [NOT NOW] Comment-only access (the Viewer/Commenter/Editor tier) — deliberately dropped from this artifact (2026-07-10); lives with parent O6/PQ2, revisit when the hosted store + directory exist and a real non-editor persona shows up.
...
| CX2 | Access tiers + GitHub boundary — "view / comment / edit, and how does that work with git?" | Cross-cutting | P0 | Parked | **Inherited (parent O6 + XQ4).** Commenting's slice: threads carry no git representation (CX1), so comments never conflict with git; any future roles enforce at the hosted sync gateway, never in files. The comment-only tier itself is dropped from this artifact — lives with parent O6/PQ2 (work-level NOT NOW). Trigger: hosted store + directory exist. (comment-model-2026-07-10) |
| CX3 | Identity + member directory dependency | Cross-cutting | P0 | Assumed | **Inherited (parent PQ1/XQ2/O2/TQ1).** Human authorship (C4) and thread mentions consume attested identity + an access-scoped member directory from the federated + member-management workstreams. Confidence: Medium. **Verify by:** confirm directory contract + owner before C4 → /spec. |
...
- **GitHub / access boundary (CX2)** — threads carry no git representation; roles enforced at the hosted sync gateway, never inside files; git-only collaborators see clean markdown and no comments (a deliberate trade-off to surface in sharing UX). Touches C4.
- **Identity + directory (CX3)** — one access-scoped directory of attested workspace principals, reconciled with git-derived authors. Touches C4. Owner: member-management + federated workstreams; this work states requirements only.
...
**Cross-workstream gates:** C4 is blocked by (a) the federated-collaboration-sync backend (Draft spec, Andrew) and (b) the member-management directory contract (CX3 — verify owner + contract before C4 → /spec).
